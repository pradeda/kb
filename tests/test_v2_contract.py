...
# The merged-bilingual union reference lives in the separate kb-eval repo, which is
# not a dependency of this deployment. Point KB_EVAL_REFERENCE at
# kb-eval/run_merged_bilingual_eval.py to run the cross-repo assertion; otherwise it
# is skipped with the resolved path in the reason.
EVAL_REFERENCE = Path(
    os.environ.get(
        "KB_EVAL_REFERENCE",
        Path(__file__).parents[1].parent / "kb-eval" / "run_merged_bilingual_eval.py",
    )
)


class FakeTokenizer:
    """Character offsets suffice for short contract fixtures; no model download."""
...
""",
    ...
    @unittest.skipUnless(
        EVAL_REFERENCE.is_file(),
        f"merged-bilingual eval reference not present at {EVAL_REFERENCE}; "
        "set KB_EVAL_REFERENCE to kb-eval/run_merged_bilingual_eval.py to enable it",
    )
    ...
    def test_router_config_is_normative_and_missing_config_fails_closed(self) -> None:
        ...
        self.assertEqual(config.router_version, "corpus-router-v2-fts5-hybrid")
        ...
        self.assertTrue(config.fts5_enabled)
        self.assertEqual((config.fts5_semantic_k, config.fts5_lexical_k), (20, 5))
        with patch.dict(
            os.environ,
